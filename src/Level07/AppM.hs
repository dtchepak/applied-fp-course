...
instance Applicative (AppM e) where
  ...
  AppM f <*> AppM a = AppM (liftA2 (liftA2 (<*>)) f a)

--  AppM f <*> AppM a = AppM $ \env -> (<*>) <$> (f env) <*> (a env)
--                    = AppM $ \env -> liftA2 (<*>)(f env) (a env)
...
instance Monad (AppM e) where
  ...
  AppM a >>= f = AppM $ \env ->
    a env >>= either (pure . Left) (flip runAppM env . f)
...
instance MonadError e (AppM e) where
  ...
  catchError (AppM a) f = AppM $ \env ->
    a env >>= either (flip runAppM env . f) (pure . pure)
