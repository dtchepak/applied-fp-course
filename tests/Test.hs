...
import           Level07.Types      (Conf(..), Port(..), DBFilePath(..))

initDb :: IO DB.FirstAppDB
initDb =
    DB.initDB (DBFilePath ":memory:") >>= either (error . show) pure

conf :: Conf
conf = Conf (Port 3000) (DBFilePath ":memory:")

env :: DB.FirstAppDB -> Env
env = Env (const (pure ())) conf

main :: IO ()
main = initDb >>= \db ->
    defaultMain $ testGroup "Applied FP Course - Tests"

  [ testWai (Core.app (env db)) "View Topic" $
      get "fudge/view" >>= assertStatus' HTTP.status200

  , testWai (Core.app (env db)) "Empty Comment" $ do
      resp <- post "fudge/add" ""
      assertStatus' HTTP.status400 resp
      assertBody "Empty Comment" resp

  , testWai (Core.app (env db)) "Invalid Route" $ do
     resp <- post "/add" "comment"
     assertStatus' HTTP.status404 resp
     assertBody "Unknown Route" resp

  , testWai (Core.app (env db)) "List Topics" $
      get "/list" >>= assertStatus' HTTP.status200

  , testWai (Core.app (env db)) "Invalid HTTP method" $ do
      resp <- put "/list" ""
      assertStatus' HTTP.status404 resp
      assertBody "Unknown Route" resp

  , testWai (Core.app (env db)) "List Posted Comments" $ do
  ...
  , testWai (Core.app (env db)) "List Topics" $ do
      post "puppies/add" "are awesome"
